...
**Hallazgo**: 3 registros de `cat_estudios` tienen datos de origen sucios, importados tal cual desde el Excel maestro (`portafolio-dev-2026/blocklabgd/v1.2/insumos-laesh/04-septiembre/LISTA 2026 PAGINA BUENAS (1).xlsx`, columna "Nombre") vía `setup/bds/laesh/migrations/generate_import_sql.py` — mismo `created_at: 2026-09-17 15:31:17` en los 3, confirmando origen en el mismo import masivo, no captura individual vía CMS:

| Clave | `nombre` actual en BD | `categoria_id` actual |
|---|---|---|
| 4311 | `MARTES: PERFIL GINECOLOGICO COMPLETO` | 18 ("LAESH", categoría genérica/placeholder) |
| 4312 | `MIERCOLES: PERFIL DE LÍPIDOS` | 18 ("LAESH", categoría genérica/placeholder) |
| 2293 | `Perfil 24` | 4 (Química Sanguínea — categoría sí es correcta, solo el nombre es vago) |

**Causa raíz**: alguien en el laboratorio escribió notas de programación/calendario ("se procesa los martes"/"miércoles") directamente en la celda de nombre del estudio en el Excel, en vez de en una columna separada, y usó "LAESH" como grupo en vez de la categoría clínica real. El script de import no tiene limpieza para este patrón (solo tiene un fix puntual para "15/24"), así que lo copió verbatim.

**Pendiente**: el usuario corregirá nombre + categoría de estos 3 estudios desde el CMS (`admrc`) directamente. Si se vuelve a correr `generate_import_sql.py` desde el Excel original sin corregir la fuente, el problema reaparecería (el script hace `TRUNCATE TABLE cat_estudios` antes de reimportar) — recordar esto si se planea un reimport futuro.
...
### P-LAESH-CSS-INVENTORY-01 🟢 [LAESH Bloc Digital] Inspección e inventario de Archivos CSS — tabla corregida; falta auditoría de reglas muertas internas
**Estado**: Pasos 1-2 y 4 completados (2026-09-21, Claude Code). Paso 3 (reglas muertas dentro de cada archivo) sigue pendiente, sin fecha.

**Hecho**: verificados los 14 archivos CSS reales en `laesh-web-assets-uipv1a/css/` contra las vistas PHP actuales. Los 2 archivos "sospechosos" (`perfil-medico.css`, `aviso-privacidad.css`) ya no existen — eliminados desde 2026-08-30, la sospecha estaba resuelta desde antes, solo la tabla nunca se actualizó. `docs.css` sí existe, se mantiene activamente (modificado 2026-09-20), y sí se carga — la hipótesis de "remanente legacy" era incorrecta para este archivo; su descripción en la tabla era errónea (decía "Docs internos", en realidad es CSS de `rc/views/solicitud_dac_impr.php`). 4 archivos reales faltaban por completo en la tabla: `ckeditor5.css`, `ckeditor-content.css`, `log-viewer.css`, `tablet-samsung-tabs10ultra.css`. **Los 14 archivos se cargan desde al menos una vista PHP — cero huérfanos a nivel de archivo**, mismo resultado limpio que la auditoría de JS. Tabla de `Especificacion_Tecnica.html` §2.3.1.1 corregida con los 14 archivos reales, sus loaders y descripciones correctas + nota de auditoría.

**Pendiente (no incluido en esta pasada, usuario decidió diferirlo)**: auditar reglas muertas *dentro* de cada archivo cargado (selectores que ya no matchean nada en el HTML actual) — como se hizo con `catalog-builder.js` (~19% código muerto real encontrado ahí). Retomar cuando el usuario lo pida explícitamente.
...
| Fecha | Item | Detalle |
|---|---|---|
| 2026-09-21 | LAESH KVM2 — `$server->push()` roto (SW_ERROR_WEBSOCKET_PACK_FAILED/8505), resuelto | Causa raíz real: Swoole 6.2.2 se compiló SIN soporte zlib (`zlib1g-dev` faltaba como dependencia en `03_install_swoole.sh` — omisión silenciosa de PECL, sin warning) → `push()` fallaba al intentar negociar permessage-deflate porque todo cliente WS estándar lo anuncia en el handshake por defecto. Fix: agregado `zlib1g-dev` + chequeo de idempotencia que ahora también verifica presencia de zlib (`php8.3 --ri swoole`), no solo versión. Recompilar reveló 2 bloqueos adicionales del propio host hardened: (a) PECL rehúsa reinstalar una versión ya registrada sin `-f` (agregado); (b) PECL no puede correr en absoluto porque `popen()` está deshabilitado (hardening de seguridad de este proyecto) y `OS_Guess::_fromGlibCTest()` lo requiere — resuelto reinstalando con un wrapper `PHP_PEAR_PHP_BIN` que reactiva `popen()` solo para esa invocación puntual (nunca tocó el `php.ini` real de producción) + deshabilitando temporalmente `20-swoole.ini` del CLI (para que PECL no se rehusara a instalar sobre un módulo que su propio intérprete ya tenía cargado). Verificado: `php8.3 --ri swoole` → `zlib => 1.3`; suite completa de 4 escenarios de negocio (nueva_orden/orden_actualizada/resultado_disponible/catalogo_actualizado) vía WS real contra producción → 12/12 verificaciones pasando. Datos de prueba (LAESH-00016, LAESH-00017 + pacientes/contadores asociados) limpiados. Logging DEBUG temporal en `swoole_server.php` (activado para este diagnóstico) removido y redesplegado — vuelto a confirmar 12/12 sin la instrumentación. Cambios en `03_install_swoole.sh` (zlib1g-dev + `-f` + chequeo de idempotencia con zlib) y `swoole_server.php` (debug removido) — pendientes de commit, esperando instrucción explícita del usuario. **Continuación misma fecha — auditoría de cobertura por transición de estado:** con el transporte ya verificado, se auditó la máquina de estados real (`1→2→3→4`, `1→5` desde RC o Médico) y se encontró que `POST /orden/cancelar` del portal Médico cambiaba el estado a Cancelada en BD pero nunca notificaba a Recepción/Admin (ni WS ni polling — `MD\Negocio\Ordenes::cancelarOrdenPropia()` llamaba a `cambiarEstado()` sin pasar por `Notifier::persist()`). Fix aplicado en `md/index.php` (mismo patrón H6 que `rc/index.php`), desplegado y verificado en vivo. Suite de pruebas extendida a 4 escenarios más (3→4 Entregar y Cerrar, 1→5 Cancelar por RC, `nueva_orden` vía Solicitud Digital de Médico, 1→5 Cancelar por Médico) → **suite final 18/18**, cobertura completa de la máquina de estados y ambos orígenes de `nueva_orden`. Datos de prueba (LAESH-00018/19/20) limpiados. Documentado en README + Especificacion_Tecnica.html (Tabla 9c fila 14) + Tecnica_Infraestructura_Despliegue.html §24.11 (G-SWOOLE-09). `md/index.php` también pendiente de commit. **Continuación misma fecha — auditoría de seguridad/QoS post-estabilización:** se encontró XSS almacenado en `ws-client.js` (`item.innerHTML = ... + data.titulo + ... + data.mensaje + ...` sin escapar) — texto de usuario (nombre de paciente, motivo de cancelación, nombre de archivo PDF) llega sin sanitizar desde `notifier.php`/callers hasta ese sumidero. CSP de producción permite `script-src 'unsafe-inline'`, así que un payload tipo `<img src=x onerror=...>` se ejecutaría sin restricción en la sesión de cualquier Admin/RC/Médico conectado. Preexistente (no introducido hoy), pero el fix del Gap 14 de hoy replicó el mismo patrón sin escapar (`$observacion` en `md/index.php`), y la estabilización de `push()` lo vuelve explotable en tiempo real (antes solo llegaba por polling). Fix: `escapeHtml()` agregado en `ws-client.js`, único punto de renderizado — cubre los 4 eventos y ambos caminos de entrega. Verificado con test XSS real vía jsdom (payload neutralizado, `onerror` nunca se ejecuta) + suite de routing de paneles sin regresión. Desplegado a producción, hash verificado. `ws-client.js` también pendiente de commit. **Continuación misma fecha — bugs reales en Solicitud Digital del portal Médico, reportados por el Dr. Hedilberto probando en producción (orden real LAESH-00026, paciente Jose Garcia Lopez):** (1) confirmado que clicks repetidos en el ícono de imprimir del documento NO duplican solicitudes — es `window.print()` puro, sin request al servidor; (2) **Bug crítico de pérdida de datos**: el checkbox de estudios en `medicos.js`/`populateMandatoryGrid()` se llamaba `name="estudio_item"` pero el backend (`md/negocio/Ordenes.php::crearSolicitudDigital()`) lee `$_POST['estudios']` — cualquier estudio marcado por el médico se perdía en silencio (orden se creaba igual, con `estudios: []` en BD, verificado contra la fila real de LAESH-00026). Fix: renombrado a `name="estudios[]"`, verificado end-to-end (nueva orden de prueba guardó el array de estudios correctamente en BD). (3) `verSolicitudDigital()` no recibía `diagnostico` al reabrir una solicitud desde la tabla — agregado el parámetro + los 3 call sites actualizados, con fuente confiable vía atributos `data-diagnostico`/`data-estudios` en el enlace del folio (`medicos.php`) en vez de adivinar por índice de celda (el índice viejo `cells[5]` leía la columna Estado, no Estudios). (4) `solicitud-dac.js`: default `'—'` para diagnóstico dejaba inalcanzable el mensaje "Sin diagnóstico especificado." — corregido. Archivos: `medicos.js`, `solicitud-dac.js`, `medicos.php` — pendientes de commit. **Purga completa de BD + reset de folio autorizado explícitamente por el usuario** una vez resueltos los 3 issues: `ordenes`, `pacientes`, `detalle_ordenes`, `historial_estados_orden`, `notificaciones`, `resultados_pdf` truncados a 0; `folios_control.ultimo_folio` reseteado a 0; `perfiles_medicos.total_ordenes` reseteado a 0 en los 5 médicos. Backup de seguridad previo a la purga: `/home/sysadmin/backup_pre_purga_20260921_213155.sql` en KVM2. Verificado en vivo: próxima orden real generó `LAESH-00001` correctamente. Sistema queda listo para uso real desde folio 1. **Continuación misma fecha — corrección de raíz GAP-RC-01 + auditoría de fragilidad + 2 bugs reales adicionales, en respuesta a pedido explícito del usuario de "realineación de integridad de datos entre flujos":** (A) **Arquitectura**: se implementó lo que `solicitud_dac_impr.php` ya documentaba como pendiente — nuevo método `RC\Negocio\Ordenes::obtenerOrdenPorFolio()` + 2 endpoints (`GET /laesh/rc/api/orden` sin restricción, `GET /laesh/md/api/orden` con ownership) que consultan `vw_ordenes_completas`+`vw_medicos_completos` como fuente única de verdad. `solicitud-dac.js` reescrito: ya no arma ~14 parámetros de querystring a mano, solo recibe `id`+`portal` y hace `fetch()`. `verSolicitudDigital()` en `medicos.js`/`labadmin.js` pasó de 8-11 parámetros a uno solo — eliminadas ~90 líneas de plumbing frágil (extracción por `cells[N]`, armado manual de cédula/universidad/lugar). (B) **Auditoría de fragilidad en todo el código de los 3 portales**: encontrado y corregido `(int)(matchObj.estado_id || 1)` en `labadmin.js` — sintaxis de PHP inválida en JS, `int` nunca definido, rompía con `ReferenceError` el buscador rápido de Admin/RC en silencio cada vez que el resultado coincidía con la caché de búsqueda. Confirmado que el resto de los 3 portales no tiene el mismo patrón (gestion-web.js/admrc limpio). (C) **Código muerto eliminado** (`labadmin.js` + `labadmin.php`): `#modal-resultados`, `verResultados()`, `cerrarModal()`, rama `'ver-resultados'` de `_orderAction`, `window.__RECEPCION_CACHE__` (nunca poblada) — ninguno era alcanzable desde el DOM real. (D) **Bug real reportado en vivo**: cache-busting estático (`?v=20260918_v113`) en `solicitud_dac_impr.php` causaba que el navegador sirviera JS viejo en caché tras cada cambio — cambiado a `?v=<?= time() ?>` en los 7 `<script>`/`<link>` de esa vista, igual que el resto del sistema. (E) **Segundo bug real reportado en vivo**: botón "Ver Resultados" de la tabla "Hoy" del portal Médico (en 2 lugares — `medicos.php` carga inicial y `md/index.php` partial HTMX) pasaba el **folio** (`LAESH-00003`) a `/laesh/md/orden/pdf?id=...`, pero esa ruta hace `(int)$_GET['id']` — `(int)"LAESH-00003"` = 0 → 404 vacío. La tabla "Anteriores" ya usaba el patrón correcto (href directo con ID numérico); unificado ahí, eliminando la indirección JS (`verResultados()`/`_medOrderAction` 'ver-resultados' también eliminados por dead-code, mismo patrón que labadmin.js). Verificado en vivo con curl: `200` tras el fix. (F) **Feature nueva**: al hacer clic en un ítem del panel de notificaciones, se marca "leído" (opacidad reducida, clase `notif-leido`, idempotente) — el ítem NO se elimina del historial (corrección explícita del usuario tras una primera implementación incorrecta que sí lo quitaba), pero el contador de esa sección (Hoy/Ayer y Anteriores/Catálogo Actualizado) y el globito de la campanita decrementan en 1, reflejando solo no-leídos. Verificado con DOM simulado en Médico (3 secciones) y Recepción. (G) **Investigación de datos**: 3 estudios del catálogo con nombres sucios (`MARTES: PERFIL GINECOLOGICO COMPLETO`, `MIERCOLES: PERFIL DE LÍPIDOS`, `Perfil 24`) confirmados como import verbatim del Excel maestro (`generate_import_sql.py`, mismo `created_at` en los 3) — no es bug de código, es dato de origen; usuario decidió corregirlo él mismo desde el CMS (ver P-LAESH-CATALOGO-NOMBRES-SUCIOS-01). Archivos nuevos/modificados esta continuación: `rc/negocio/Ordenes.php`, `rc/index.php`, `md/index.php`, `solicitud-dac.js`, `medicos.js`, `labadmin.js`, `medicos.php`, `labadmin.php` — todos pendientes de commit. |
...
**Contexto original**: durante prueba real en producción (Recepción no recibía notificaciones en vivo, requería refresh manual), se corrigió un bug real en `ws-client.js` (loop infinito de reconexión que nunca activaba el fallback de polling). Fix desplegado y verificado — ver detalle histórico abajo.

**2026-09-23 — hipótesis "Cache::clear() de las 5am borra JTI_*" DESCARTADA**: se confirmó con `nginx-access.log` histórico (retención hasta 6-sep) que `Cache::clear()` (cron `cache_renew.php`, 5:00 AM) sí borraba `JTI_*` además de sus 4 claves de diseño — **bug real, corregido** (`GAP-CACHE-01`, `commons/Cache.php` — `clear()` ahora usa `invalidate([KEY_CFG, KEY_CMS, KEY_TREE, KEY_PROMOS])`, ya no toca `JTI_*`). Pero el patrón real observado en logs **no coincide** con esa hipótesis: sesión real de personal (IP 189.238.227.84, 19-sep, sin contaminación de pruebas propias) mostró **100% de rechazo WS desde 1 segundo después de un login recién emitido** (JWT recién creado — no puede tener caché expirada de 5am) y repitiéndose sin cambiar durante toda la sesión (~190 intentos en 12.5 min). El mismo usuario/IP tuvo 0% de rechazo en otros días. Es decir: falla completa desde el primer intento en *algunos* eventos de login, no degradación gradual por antigüedad de caché — la causa raíz real sigue siendo otra cosa, probablemente algo en el camino inmediato login→primer WS (client-side timing, o alguna otra condición de carrera aún no identificada).

**2026-09-23 — auditoría permanente desplegada** (reemplaza la necesidad de instrumentación temporal en vivo): `verifyWsJwt()` en `swoole_server.php` ahora retorna siempre el motivo exacto de rechazo (`empty_token | malformed_token | invalid_signature | invalid_payload | expired | jti_cache_miss | jti_revoked`) en vez de un `null` opaco. Cada rechazo se audita vía el mismo puente HTTP inverso que `ws_conexiones_log` (`auditWsRejection()` → `/internal/ws-audit` → tabla nueva `ws_rechazos_log`, con `jti`/`user_id` nullable ya que varios motivos ocurren antes de poder leer el payload). Código desplegado y verificado en KVM2 (checksums OK) — **falta crear la tabla en la BD de producción** (SQL idempotente subido a `/tmp/ws_rechazos_log.sql` en KVM2, pendiente de que el usuario lo ejecute con sus credenciales — Claude Code no tiene acceso de escritura a la BD de prod). Una vez creada la tabla, la próxima ocurrencia real queda capturada con motivo exacto + jti + user_id + IP, sin necesitar reproducir nada en vivo.

**Próximo paso real**: una vez la tabla exista y ocurra un nuevo rechazo real, `SELECT * FROM ws_rechazos_log ORDER BY id DESC` da el motivo exacto directamente — si es `jti_cache_miss` repetido en logins nuevos, apunta a un problema de escritura/lectura de caché en el instante del login (no de expiración); si es otro motivo, redirige la investigación por completo.

**Impacto actual**: bajo — el fix de `ws-client.js` ya evita que esto afecte al usuario (máx. ~4-7s de retraso vía polling en vez de indefinido).

<details><summary>Detalle histórico del fix de síntoma (2026-09-22)</summary>

Se encontró y corrigió un bug real en `ws-client.js`: el contador `reconnectAttempts` se reseteaba en cada `onopen`, incluso cuando el servidor cerraba la conexión casi de inmediato (`swoole_server.php` → `on('open')` → `verifyWsJwt()` falla → `$server->close($request->fd)`) — esto generaba un loop infinito de reconexión (1247 reconexiones confirmadas en logs de una sola sesión) que **nunca** alcanzaba `maxReconnects` y por lo tanto **nunca** activaba el fallback de polling HTTP (`startPollingFallback()`, diseñado para entregar notificaciones cada 4s si WS falla). Resultado: cero actualizaciones en vivo hasta refresh manual.
...
Lo que se creyó sin resolver en su momento — por qué `verifyWsJwt()` rechazaba la conexión de Recepción repetidamente, pese a que el archivo de caché del JTI existía con `'revoked' => false`, `LAESH_CACHE_DIR` coincidía entre pool PHP-FPM y systemd de swoole, y el directorio de caché era accesible. La hipótesis de rotación de JTI en caché quedó descartada el 2026-09-23 (ver arriba).
