...
| **F05** | **Semántica dual del campo `recargo` — NUNCA contabilizar como monto financiero** — El campo `recargo` existe con dos semánticas incompatibles según la tabla: (1) En `cargos` (catálogo): es un **flag entero `INT(0/1)`** que indica si el tipo de cargo puede generar recargo moratorio. (2) En `ligacargos` / `ligacargos_historico`: es un **monto decimal heredado** del catálogo al momento del INSERT — en Host C vale `0.00` para casi todos los registros; en datos migrados de Host A/B puede valer `1.00` (artefacto histórico de cuando el flag se copió como float). **Regla crítica**: ningún reporte, vista ni lógica de negocio debe sumar `ligacargos.recargo` como parte de la deuda o ingreso. El discriminador canónico para identificar recargos moratorios es `categoria IN (16, 17)` (valores configurables en `config_sistema`: `recargo_categoria_agua`, `recargo_categoria_drenaje`). La vista `vw_ligacargos_pendientes` y `vw_ligacargos_all` exponen el alias `es_recargo_moratorio = IF(categoria IN (16,17), 1, 0)` — usar siempre ese campo para clasificar. El `recargo` decimal en registros de cat 6 (asamblea) que vale `1.00` en datos históricos no representa deuda real y no afecta ningún total financiero. | Implementada |
